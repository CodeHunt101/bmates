...
export const Message = ({ message, currentUser }) => {
  ...
  const renderImageUrl = () => (
    currentUser.current_user.id === message.sender_id ? currentUser.profile_picture_url : findSender()?.sender_profile_picture
  )

  const renderMessageDateTime = () => (
    `${messageDate.toLocaleDateString()} at ${messageDate.toLocaleTimeString('en-US', {hour: '2-digit', minute: '2-digit'})}`
  )
